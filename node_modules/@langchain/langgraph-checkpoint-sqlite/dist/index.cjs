...
const better_sqlite3 = require_rolldown_runtime.__toESM(require("better-sqlite3"));
  ...
  SELECT
    ...
    (
      ...
      WHERE ps.thread_id = checkpoints.thread_id
        ...
        AND ps.channel = '${__langchain_langgraph_checkpoint.TASKS}'
      ORDER BY ps.idx
    ) as pending_sends
  FROM checkpoints
...
var SqliteSaver = class SqliteSaver extends __langchain_langgraph_checkpoint.BaseCheckpointSaver {
	db;
	isSetup;
	withoutCheckpoint;
	withCheckpoint;
	...
	static fromConnString(connStringOrLocalPath) {
		return new SqliteSaver(new better_sqlite3.default(connStringOrLocalPath));
	}
	setup() {
...
CREATE TABLE IF NOT EXISTS writes (
  ...
  PRIMARY KEY (thread_id, checkpoint_ns, checkpoint_id, task_id, idx)
	...
	async put(config, checkpoint, metadata) {
		...
		const preparedCheckpoint = (0, __langchain_langgraph_checkpoint.copyCheckpoint)(checkpoint);
	...
	async migratePendingSends(checkpoint, threadId, parentCheckpointId) {
		...
		mutableCheckpoint.channel_values[__langchain_langgraph_checkpoint.TASKS] = await Promise.all(JSON.parse(pending_sends).map(({ type, value }) => this.serde.loadsTyped(type, value)));
		mutableCheckpoint.channel_versions[__langchain_langgraph_checkpoint.TASKS] = Object.keys(checkpoint.channel_versions).length > 0 ? (0, __langchain_langgraph_checkpoint.maxChannelVersion)(...Object.values(checkpoint.channel_versions)) : this.getNextVersion(void 0);
	}
};
