...
export const NeuralHexNode: React.FC<NeuralHexNodeProps> = ({ 
    ...
    return (
        ...
        >
            <motion.div
                className="relative flex items-center justify-center bg-void-900/40 backdrop-blur-sm border border-white/5 cursor-pointer transition-all duration-300 w-full h-full"
                style={{ 
                    clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)', // Pointy top
                }}
                animate={{
                    borderColor: isHovered ? agent.color : 'rgba(120, 130, 255, 0.3)',
                    borderWidth: isHovered ? '2px' : '1px',
                    boxShadow: isHovered ? `0 0 30px ${agent.color}40` : 'none',
                    backgroundColor: isHovered ? 'rgba(7, 7, 10, 0.9)' : 'rgba(7, 7, 10, 0.4)'
                }}
            >
                {/* Inner tint */}
                <div 
