...
const DEFAULT_PRICING = { inputPer1M: 5.00, outputPer1M: 15.00 };
// ============================================================================
// DATABASE
// ============================================================================
...
export function initCostTrackingTable() {
...
export function logUsage(input) {
    ...
    database.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, input.agentName, input.model, input.tokensIn, input.tokensOut, costUsd, input.latencyMs, input.threadId || null, input.userId || null, input.metadata ? JSON.stringify(input.metadata) : null, createdAt);
    return {
        id,
...
let dailyCostStmt = null;
export function getDailyCost(dateStr) {
    const database = getDb();
    ...
    const row = dailyCostStmt.get(dayStart, dayEnd);
    return {
        date: targetDate,
 ...
 */
export function getAgentCosts(startDate, endDate) {
    const database = getDb();
 ...
 */
export function getModelCosts(startDate, endDate) {
    const database = getDb();
    const start = startDate || getDateOffset(-30);
 ...
 */
export function getCostSummary(period = "week") {
 ...
 */
export function formatCostSummary(period = "week") {
 ...
 */
export function getTopQueries(limit = 10, startDate, endDate) {
    const database = getDb();
    const start = startDate || getDateOffset(-30);
    const end = endDate || getDateOffset(1);
    const rows = database.prepare(`
 ...
 */
export function getCostProjection(daysToAnalyze = 7) {
    const database = getDb();
 ...
 * @internal
 */
export function _resetDb(newDb) {
    db = newDb || null;
}
