 ...
 */
export async function storeMemory(content, agent, metadata) {
    const id = randomUUID();
    // Generate embedding
    ...
    const stmt = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
    `);
    stmt.run(id, agent, content, Buffer.from(new Float32Array(vector).buffer), metadata ? JSON.stringify(metadata) : null);
    return id;
}
/**
...
export function formatMemoriesForPrompt(memories) {
    ...
    const formatted = memories.map((m, i) => `${i + 1}. [${m.agent}] (${(m.similarity * 100).toFixed(0)}% relevant):\n${m.content.substring(0, 300)}...`).join("\n\n");
    return `
## 📚 Relevant Past Work
