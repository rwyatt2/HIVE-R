...
workflow.addConditionalEdges("Builder", (state) => {
    ...
    if (state.next && HIVE_MEMBERS.includes(state.next)) {
...
function createAgentRouter(agentName) {
    return (state) => {
        // Check for direct handoff via state.next
        if (state.next && state.next !== "Router" && HIVE_MEMBERS.includes(state.next)) {
...
for (const member of HIVE_MEMBERS) {
    if (member !== "Builder") {
        workflow.addConditionalEdges(member, createAgentRouter(member), {
            Router: "Router",
