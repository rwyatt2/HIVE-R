...
export function checkTurnLimit(turnCount: number): { safe: boolean; reason?: string } {
...
export function checkAgentRetries(
    agentName: string,
...
export function isCircuitOpen(agentName: string): boolean {
    const cb = circuitBreakers[agentName];
    if (!cb) return false;
...
export async function withTimeout<T>(
    promise: Promise<T>,
    timeoutMs: number = SAFETY_CONFIG.AGENT_TIMEOUT_MS,
...
export async function safeAgentExecution<T>(
    agentName: string,
    fn: () => Promise<T>,
    fallback: T
): Promise<T> {
