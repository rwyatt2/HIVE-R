...
${CONTEXT_PROTOCOL}`;
export const uxResearcherNode = async (state) => {
    ...
    try {
                ...
                try {
                    switch (toolCall.name) {
                        case "web_search":
                            ...
                            break;
                        case "fetch_url":
                            result = await fetchUrlTool.invoke(toolCall.args);
                            break;
                        default:
        ...
        console.error("❌ UXResearcher failed:", error);
        return {
            messages: [
                new HumanMessage({
...
//# sourceMappingURL=ux-researcher.js.map