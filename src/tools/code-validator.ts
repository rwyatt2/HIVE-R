...
export const eslintValidateTool = tool(
    async ({ filePath }) => {
        ...
        try {
            ...
            const { stdout, stderr } = await execAsync(
                `npx eslint "${fullPath}" --format json 2>/dev/null || true`,
                { cwd: WORKSPACE }
            );
            ...
            try {
                ...
                const formatted = issues.map((msg: { severity: number; line: number; message: string; ruleId: string }) => {
...
export const typescriptCheckTool = tool(
    async ({ filePath }) => {
        const fullPath = path.isAbsolute(filePath) ? filePath : path.join(WORKSPACE, filePath);

        try {
            const { stdout, stderr } = await execAsync(
                `npx tsc --noEmit --skipLibCheck "${fullPath}" 2>&1 || true`,
                { cwd: WORKSPACE }
            );
...
export const accessibilityCheckTool = tool(
    async ({ code }) => {
        const issues: string[] = [];
...
export const bundleSizeEstimateTool = tool(
    async ({ dependencies }) => {
